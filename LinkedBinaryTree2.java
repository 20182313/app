...
public class LinkedBinaryTree2<T> implements BinaryTreeADT<T>, Iterable<T>
{
    ...
    protected LinkedBinaryTree2<T> left,right;
    ...
    public void removeRightSubtree(LinkedBinaryTree2<T> node)
    {
    ...
    public void RebuildTree(T[] inorder, T[] preorder) {

        BinaryTreeNode temp =
                makeTree(inorder, 0, inorder.length, preorder, 0, preorder.length);
        root = temp;

    }


    private   BinaryTreeNode<T> makeTree(T[] inorder,int first1,int num1,T[] preorder,int first2,int num2){
        if(num1 < 1)
        {
            return null;
        }
        ...
        T rootelement = preorder[first2];//先序遍历的第一个元素为根节点
        ...
        for(i = 0;i < num1;i++)
        {
            if(inorder[first1+i] == rootelement)
            {
                isFound=true;
                break;
            }
        ...
        //中序遍历中，根节点的左边放入左子树，右边的放入右子树，
        // 再通过迭代在左右子树中确定元素位置
        root.setLeft(makeTree(inorder, first1, i, preorder, first2+1, i));

        //
        root.setRight(makeTree(inorder, first1+i+1, num1-i-1,
                preorder, first2+i+1, num2-i-1));
        return root;
    }
}